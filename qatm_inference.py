...
from clip_sim import Clipinference
...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(self, template_dir_path, image_name, thresh_csv=None, transform=None):
        ...
        self.template_path = template_dir_path
        self.image_name = image_name

        self.image_raw = cv2.imread(image_name)
...
template_path = '/home/all/ljc/1226_machine/small_jpg/web/1400X1050_1440X900/4/_small_jpg/8.jpg'
image_path = '/home/all/ljc/1226_machine/small_jpg/web/1400X1050_1440X900/4/2.png'
template = cv2.imread(template_path)
template = transform(template)
template = template.unsqueeze(0)
image = transform(cv2.imread(image_path)).unsqueeze(0)
# dataset = ImageDataset(template_dir, image_path, thresh_csv='thresh_template.csv')
# model, template, image, image_name
...
class QATM():
    ...
    def __call__(self, x):
        ...
        conf_values, ind3 = torch.topk(confidence.cpu(), 1)
...
while True:
    start = time.time()
    score = run_one_sample(model, template, image, image_path)
    h_array = template.size()[-2]
    w_array = template.size()[-1]
    thresh_list = 0.7
    scores = []
    w_arrays = []
    h_arrays = []
    ...
    # np.squeeze(np.array(scores), axis=1), np.array(w_array), np.array(h_array), thresh_list

    print('2')
    boxes, indices = nms_multi(np.squeeze(np.array(scores), axis=1), np.array(w_arrays), np.array(h_arrays), thresh_lists)
    print('3')
    end_t = time.time()

    print(end_t-start)
    d_img = plot_result_multi(cv2.imread(image_path), boxes, indices, show=False, save_name='result_sample.png')
# print('4')
# plt.imshow(scores[2])
